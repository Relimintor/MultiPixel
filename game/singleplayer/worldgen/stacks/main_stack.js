(function () {
  ...
  class MainBiomeStack {
    ...
    sampleBiomeStack(wx, wz, baseLand, baseTemp, hillNoise) {
      let biome = this.ops.temperatureToBiome(baseTemp, wx, wz, 256);
      if (this.settings.enableBambooJungleVariant) biome = this.ops.bambooJungleVariant(biome, wx, wz, 256);
      biome = this.ops.biomeEdge(biome, wx, wz, 64);
      biome = this.ops.regionHills(biome, hillNoise, wx, wz, 64);
      if (this.settings.enableSunflowerPlainsVariant) biome = this.ops.sunflowerPlainsVariant(biome, wx, wz, 64);
      if (baseLand !== C().LAND && baseLand !== 99) biome = baseLand === C().DEEP_OCEAN ? 'Deep Ocean' : 'Ocean';
      biome = this.ops.shore(biome, wx, wz, 16);
      biome = this.ops.smoothBiome(biome, wx, wz, 4);
      return biome;
    }
  }
